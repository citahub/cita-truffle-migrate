...
const log = console.log.bind(console, '--- resolver fs.js >>>\n')
...
FS.prototype.getContractName = function(sourcePath, searchPath) {
  ...
  for (var i = 0; i < filenames.length; i++) {
    ...
    var artifact = JSON.parse(fs.readFileSync(path.resolve(searchPath, filename)))
...
FS.prototype.resolve = function(import_path, imported_from, callback) {
  ...
  var possible_paths = [import_path, path.join(path.dirname(imported_from), import_path)]
